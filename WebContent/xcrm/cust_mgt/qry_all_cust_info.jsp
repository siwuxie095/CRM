...
<%@ include file="/xcrm/bootstrap/nav.jsp" %>
	...
	<%
		...
		ResultSet rs=custInfoDao.getAll(conn);
	...
	<div class="row-fluid">
		<div class="span12">
			...
			<a href="#myModal" role="button" class="btn" data-toggle="modal"><span class="add-on"><i class="icon-search"></i></span>&nbsp;查找</a>
					...
					<a href="<%= request.getContextPath() %>/xcrm/cust_mgt/qry_all_cust_info.jsp" class="btn"> <span class="add-on"><i class="icon-repeat"></i></span>&nbsp;刷新</a>
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr>
						<td>
							<label class="checkbox"><input type="checkbox" name="custId" value="<%= rs.getString("cust_id") %>" /></label>
						</td>
						<td>
						...
						<% if(!crmRole.equals("comm")){ %>
						<a href="<%= request.getContextPath() %>/deleteCustInfoServlet?custId=<%= rs.getString("cust_id") %>&delMode=sgl&rtnUri=<%= currentUri %>" class="btn"> <span class="add-on"><i class=" icon-trash"></i></span>&nbsp;删除</a>
						<% } %>
						</td>
					</tr>
				</tbody>
			...
			if(num==0){
				...
				if(confirm(tip)){
					...
					var custIds="";
					 ...
					 document.location.href="<%= request.getContextPath() %>/deleteCustInfoServlet?custIds="+custIds+"&delMode=mul&rtnUri=<%= currentUri %>";
					return true;
				}else{
					return false;
				}
