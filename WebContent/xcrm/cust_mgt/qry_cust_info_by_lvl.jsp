...
<%@ include file="/xcrm/bootstrap/nav.jsp" %>
	...
	<%
		...
		ResultSet rs=custInfoDao.getByLvl(conn, custInfo);
	...
	<div class="row-fluid">
		...
		<div class="btn-group table-bordered">
			...
			<a href="#myModal" role="button" class="btn" data-toggle="modal"><span class="add-on"><i class="icon-search"></i></span>&nbsp;查找</a>
					...
					<a href="<%= request.getContextPath() %>/xcrm/cust_mgt/qry_cust_info_by_lvl.jsp?custLvl=<%= custLvl %>" class="btn"> <span class="add-on"><i class="icon-repeat"></i></span>&nbsp;刷新</a>
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr>
						...
						<% if(!crmRole.equals("comm")){ %>
						<a href="<%= request.getContextPath() %>/deleteCustInfoServlet?custId=<%= rs.getString("cust_id") %>&delMode=sgl&rtnUri=<%= currentUri %>&param=<%= custLvl %>" class="btn"> <span class="add-on"><i class=" icon-trash"></i></span>&nbsp;删除</a>
						<% } %>
						</td>
					</tr>
				</tbody>
			...
			if(num==0){
				...
				if(confirm(tip)){
					...
					var custIds="";
					 ...
					 document.location.href="<%= request.getContextPath() %>/deleteCustInfoServlet?custIds="+custIds+"&delMode=mul&delMode=sgl&rtnUri=<%= currentUri %>&param=<%= custLvl %>";
					return true;
				}else{
					return false;
				}
